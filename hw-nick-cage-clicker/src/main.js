...
function init(argImageData){
	...
	// Load Sounds
	hitSound = new Howl({src: ['sounds/shoot.wav'], volume: 0.2});
	hitWrongSound = new Howl({src: ['sounds/bonk.mp3'], volume: 0.1});
	missSound = new Howl({src: ['sounds/miss.mp3'], volume: 0.2});
...
function drawHUD(ctx)
{
	...
	switch(gameState)
	{
		case GameState.START:
			...
			fillText(ctx,"By John Pionzio", screenWidth/2, screenHeight/2 + 70, "14pt 'Press Start 2P', cursive", "red");
			strokeText(ctx,"By John Pionzio", screenWidth/2, screenHeight/2 + 70, "14pt 'Press Start 2P', cursive", "white",2);
			fillText(ctx,"YOUR GOAL: To click all the Nick Cage's!", screenWidth/2, screenHeight/2 + 130, "8pt 'Press Start 2P', cursive", "red");
			fillText(ctx,"Click anywhere to begin", screenWidth/2, screenHeight/2 + 160, "8pt 'Press Start 2P', cursive", "red");
			break;
	...
	switch(currentLevel){
			...
			case 3:
				...
				sprites = sprites.concat(
					...
					createImageSprites(5, 60, 45, imageData.tommy1, "tommy", rect),
					createImageSprites(5, 50, 50, imageData.trollface, "trollface", rect),
					createImageSprites(7, 50, 71, imageData.franco1, "franco", rect),
					createImageSprites(8, 50, 70, imageData.trollface, "trollface", rect)
				);
				levelGoal = 9;
				levelTarget = "cage";
				vectorChangeProb = 0.01;
				levelTimeLimit = 15;
				break;
...
function doMousedown(e)
{
	...
	switch(gameState)
	{
		...
		case GameState.MAIN:
			...
			// Filter out the clicked "not Cage" sprites
